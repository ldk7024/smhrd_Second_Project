...
function callBack(data){
	...
	$.each(data,(index,obj)=>{ 
		view+="<tr>";
		view+="<td class='cart-pic first-row' id='board_idx"+index+"'>"+obj.board_idx+"</td>";
		view+="<td class='cart-title first-row' ><a href='javascript:contentFn("+index+")'>"+obj.board_title+"</a></td>";
		view+="<td  class='p-price first-row' >"+obj.board_count+"</td>";
		view+="<td  class='qua-col first-row'>"+obj.board_writer+"</td>";
		view+="<div class='quantity'>";
		view+="<div class='pro-qty'>";
		view+="</div>";
		view+="</div>";
		view+="</td>";
		view+="<td  class='total-price first-row'>"+obj.board_indate+"</td>";
		/* view+="<td class='close-td first-row'><i class='ti-close'></i></td>"; */
								
		/* view+="<c:if test='${sessionScope.memberVO==null || sessionScope.memberVO.member_id!=\'admin\'}'>";
		view+="<td><button class='btn btn-warning' onclick='delBtn("+obj.board_idx+")' disabled='disabled'>삭제</button></td>";
		view+="</c:if>"; */	
		view+="<c:if test='${sessionScope.memberVO.member_id==\'admin\'}'>";
		view+="<td><button class='btn btn-warning' onclick='delBtn("+obj.board_idx+")'>삭제</button></td>";
		view+="</c:if>";						 
		view+="</tr>";
	});
	view+="</tbody>";
...
function contentFn(index){
	...
	var board_idx = $("#board_idx"+index).text();
	//alert(board_idx); //데이터 잘 들어오는지 확인
	$.ajax({
		url : "ajaxcontent.do",
		type : "get",
...
function callContent(data){
	...
	var board_content=data.board_content;
	var board_writer=data.board_writer;
	$("#cidx").val(board_idx);
	$("#ctitle").val(board_title);
	$("#ccontents").val(board_content);
	$("#cwriter").val(board_writer);
			...
			<div class="row">
				<div class="col-lg-12">
					<div class="breadcrumb-text">
					    <input type="text" name="wirter" class="form-control" value="${sessionScope.MemberVO.member_id}">
